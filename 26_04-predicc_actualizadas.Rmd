...
summary(PCA_std)
```
Con las 2 primeras componentes recogemos el 60% de la variabilidad. Con 4 recogeríamos un 75%. En nuestro caso, trabajaremos con las dos primeras componentes, puesto que las siguientes no añaden de forma significante información adicional. 
...
## Conclusión
En conclusión, podemos observar que el número de clústers óptimo sería 2 o 3. El método del codo o de la silueta nos indican que debemos dividir en dos grupos y los métodos GAP y NbClust son arrojan un el mismo resultado.
...
```{r}
set.seed(123)
# Female es TP, male es TN. Female va delante al binarizar (en primer lugar se pone el yes)
...
```
*Para maximizar el F1 Score, utilizando 3 posbibles valores de números de vecinos obtenemos que para 9 vecinos se consigue el mejor valor del F1. RECUERDA: Buscamos maximizar el F1 porque al tener una porporción de 90% hombres y 10% mujeres, queremos predecir con certeza cuando una edición ha sido realizada por una mujer.*


Realizamos una predicción de k=9 en train.
**NOTA: Nuestros TP o True Possitive son las mujeres (female), mientras que los TN o True Neggative son los hombres (male).**
```{r, warning=FALSE}
#TRAIN
prediction.knn_train <- predict(fit.knn,newdata=df_kvecinos_train,type="prob")["female"]

#Binarizamos a partir de 0.2 puesto que es lo mejor entre train y test. En train es mejor con 0.1, pero en test va mucho peor con 0.1
clase.pred.knn_train=ifelse(prediction.knn_train>0.2,"female","male")

cf11 <- confusionMatrix(as.factor(clase.pred.knn_train),as.factor(df_kvecinos_train$gender),
      positive="female", mode="everything")
print(cf11)
```
*En la matriz de confusión apreciamos que de 219 mujeres, el modelo ha acertado en la predicción para 67 de ellas. El mejor valor de F1-Score que hemos conseguido, 0.46271, lo hemos conseguido binarizando con un umbral de 0.2: si se obtiene un número mayor que 0.2 entonces se predice como mujer. La precisión a la hora de predecir hombres disminuye, aunque no es algo muy importante para nosotros puesto que nuestro objetivo es detectar la mayor parte de las mujeres editoras que podamos.*

Observemos cómo se comporta el modelo con los datos de test.
```{r, warning=FALSE}
#TEST
prediction.knn_test <- predict(fit.knn,newdata=df_kvecinos_test,type="prob")["female"]

clase.pred.knn_test=ifelse(prediction.knn_test>0.2,"female","male")

cf21 <- confusionMatrix(as.factor(clase.pred.knn_test),as.factor(df_kvecinos_test$gender),
      positive="female", mode="everything")
print(cf21)
```
*El valor para el F1-Score disminuye considerablemente con respecto a train. Son especialmente notorias las caídas en los valores de precision y recall respecto a train, los cuales afectan directamente al valor de F1-Score obtenido.* 
...
prediction.knn2_train <- predict(fit.knn.grid,newdata=df_kvecinos_train,type="prob")["female"]
...
print(cf12)
```
*Se obtiene un F1-Score de 0.45434. Parece que con k=10 se obtienen unos resultados muy similares a los previamente obtenidos con k=9 en train.*

Veamos si en relación a test conseguimos mejores resultados con k=10.
```{r, warning=FALSE}
#TEST
prediction.knn2_test <- predict(fit.knn.grid,newdata=df_kvecinos_test,type="prob")["female"]
...
prediction1 <- predict(fit_dt1,newdata=train_Data,type="prob") [,2] # [,2] porque las mujeres son la segunda columna (con ["female"] no funciona, no sé por qué. En knn sí funciona con eso. Será se le metio gender de alguna forma especial. Funciona igual. 
...
Veamos cómo qué resultados obtenemos con el árbol con hiperparámetros ajustados. Veamos si su complejidad, que involucra complejidad en su comprensión, nos permite realizar mejores predicciones.
```{r,warning=FALSE}
#TRAIN
...
print(rf)
```
*La matriz de confusión arroja resultados coherentes y un error OOB del 10.17%, el cual no parece especialmente grande. Sin embargo, se indica un error del 80% para la predicción de mujeres* ¿¿¿ COMO PUEDE SER QUE SALGA ESTE ERROR???
...
print(cf.rf_test)
```
*El valor de F1-Score de 0.36257 es realmente bueno en comparación con lo obtenido con otros modelos en conjuntos de test. Sin embargo, en comparación con los resultados de train, el modelo tiene cierto sobreajuste. No todo podía salir bien.*
...
varImp(rf)
```
*Con mucha diferencia, así como nos indicaban los árboles de decisión que generamos previamente de forma individual, la variable más decisiva para realizar predicciones para el modelo de Random Forest es wikiprojWomen_bin.Un paso por debajo se encuentran NDays y NPages.*
...
varImpPlot(rf)
```
*No obtenemos conclusiones coherentes con la salida anterior.*
...
```
Cabe destacar la hoja 2. Si una observación acabó en la hoja2 (1.8144), la probabilidad media de que una observación fuera un “yes” en la variable gender es del 86%. 

Si nos fijamos en las variables con mayor importancia que se encuentran en a lo largo del camino del árbol hasta llegar a dicha hoja, se observa que las que más contribuyen a que el modelo prediga "mujer" son wikiprojWomen, NDays, NPages, Año_inicial y ns_wikipedia. 

<!--El código de predicciones de XGBoost creo que está bien aunque ponga en la matriz de confusión positive="1". Si pones positive="2" te dice que te vas de rango, así que supongo que solo acepta 1 y que lo está haciendo para mujeres. Al menos, si se baja el umbral el accuracy mejora las predicciones para mujer, subiendo el umbral mejora para hombre. Al menos este comportamiento lo es coherente con los modelos anteriores, parece que funciona igual, aunque tiene diferente sintaxis.-->
...
probabilities_bayes <- predict(model_bayes, train_dt[,-1], type="prob")[,2]
classes <- as.numeric(probabilities_bayes > 0.5)

# Compute confusion matrix using caret's confusionMatrix function
confusionMatrix(table(classes, datos$test$gender), positive = "1", mode="everything")
```
*conclusiones train*

```{r}
model <- naive_bayes(gender ~ ., data = train_dt, usekernel = T) 
# Debemos quitar el target para la predicción:test_Data[,-1] 
probabilities_bayes <- predict(model, train_dt[,-1],type="prob")[,2] 
classes <- as.numeric(probabilities_bayes>0.5)

confusionMatrix(table(classes_factor, train_dt$gender),positive="1") 
```
*conclusiones test*
...
<!-- NO ejecutar esto hasta que todas las predicciones funcionen-->

```{r}

par(pty = "s") 

#XGBoost aun no funciona (no se de donde se saca "probabilities")
...
# Decision tree: prediction3 es la prediccion en test con árbol con hiperparámetros ajustados (F1-Score=0,95)
prediction3_roc <- predict(tune.fit, test_dt, type = 'prob')[,2]
roc(test_dt$gender, prediction3_roc, percent=TRUE, col="goldenrod",lwd= 2,
    print.auc =TRUE, add=TRUE,print.auc.y = 30,plot=TRUE,legend=TRUE)


# Random Forest:clase.pred.rf_test es la predicción en test.
roc(test_dt$gender, clase.pred.rf_test, percent=TRUE, col="salmon",lwd= 2,
    print.auc =TRUE, add=TRUE,print.auc.y = 20,plot=TRUE,legend=TRUE)

# KNN: prediction.knn2_test es la predicción en test con k=10
...
# METER NAIVE BAYES SI CONSIGO QUE FUNCIONE

legend("bottom",
       legend=c("XGBOOST", "DT", "RF","KNN", ),
       col=c("#377eb8", "goldenrod","salmon","#977eb8"),
