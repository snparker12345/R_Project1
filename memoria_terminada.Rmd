...
```{r, warning=FALSE}
#TRAIN
prediction.knn_train <- predict(fit.knn,newdata=df_kvecinos_train,type="prob")[,2]
...
```{r, warning=FALSE}
#TEST
prediction.knn_test <- predict(fit.knn,newdata=df_kvecinos_test,type="prob")[,2]
...
La predicción en train fue la peor, con un valor de F1-Score de 0.39306, y la de test tampoco fue buena, con un valor de 0.27848. El modelo es más simple que los anteriores, por lo que es normal haber observado peor rendimiento. Sorprende mucho un valor de AUC tan alto. <!--  VER MAIL CARMEN AV PABLOPG-->
...
**Por tanto, teniendo en consideración todo lo analizado con anterioridad, la calidad de interpretabilidad y la complejidad de los distintos modelos, podemos concluir que el mejor modelo es el Naive Bayes.** En nuestro caso, escogeríamos el modelo ingenuo de Bayes para predecir con los datos de validación, puesto que el AUC obtenido es muy alto en comparación con la simplicidad de funcionamiento del modelo.


<!--  VER SI HAY METER LOS CAMBIOS A FUTURO QUE HABRÍA QUE HACER TAMBIÉN EN LA MEMORIA: MAIL CARMEN AV PABLOPG-->
