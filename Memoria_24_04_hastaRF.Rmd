...
![Fundación Wikimedia:https://es.wikipedia.org/wiki/Fundaci%C3%B3n_Wikimedia](img1.svg)
...
<!-- VER SI DEJAMOS ESTA PARTE O NO -->
...
Representemos NIJ
```{r}
#Histograma para ver como están repartidos los grupso en NIJ.
ggplot(data=train_Data,aes(x=weightIJ,fill=weightIJ)) +
  geom_bar(aes(y=(..count..)/sum(..count..))) +
  scale_y_continuous(labels=scales::percent) +
  scale_x_discrete(labels = NULL) + 
...
```
*Observamos que el primer cuartil es bajo, por lo que tenemos una gran concentración de datos con valores bajos. La media es muy superior a la mediana, lo que va a presentar asimetría a la derecha. También se observa que el máximo de ediciones es (101022), dato aparentemente parece erróneo.*
...
```
*Las medias para el primer año con eidición para cada género difieren, para cada hombres la media está en 2010 y para las mujeres en 2012.*
...
```
*Sale un p-valor inferior a 0.05 por lo que rechazamos la hipótesis nula.Por lo que sí existe una relación entre el año inicial y el género.*
...
```{r}
# Graficamos las PDF
grid.arrange(ggplot(train_Data, aes(x = new_ns_user, colour = train_Data$gender)) + geom_density(lwd=2, linetype=1), 
             ggplot(train_Data, aes(x = new_ns_userTalk, colour = train_Data$gender)) + geom_density(lwd=2, linetype=1), nrow=1)
```
</center>\
...
grid.arrange(ggplot(train_Data, aes(x = new_ns_content, colour = train_Data$gender)) + geom_density(lwd=2, linetype=1), ggplot(train_Data, aes(x = new_NActDays, colour = train_Data$gender)) + geom_density(lwd=2, linetype=1), nrow=1)
```
</center>\
...
grid.arrange(ggplot(train_Data, aes(x = new_ns_talk, colour = train_Data$gender)) +geom_density(lwd=2, linetype=1), 
             ggplot(train_Data, aes(x = new_ns_wikipedia, colour = train_Data$gender)) + geom_density(lwd=2, linetype=1), nrow=1)
```
</center>\
...
<!-- La información sobre las hipotesis de contraste de chisq y tstudent ponerlas la primera vez que se hagan esos test, cuando ordenemos las variables del multivariante por orden (segun como se hayan ido tratando en eda univariante). Luego no hay que estar repitiendo nada. Estan en variables wikiprojwomen y un poco antes (mías). Pasar de ahí a donde convenga y vaya primero en la memoria y cambiar lel texto que acompaña explicando los test. No hay que decir continuamente que se hacen los test. -->
...
<!-- CORREGIR ESTO CUANDO TRAIN_PCA TENGA LAS VARIABLES DEFINITIVAS Y LAS COMPONENTES PRINCIPALES TAMBIÉN LO SEAN -- AÑADIR APARTADO CONCLUSION CUANDO EL PCA SEA DEFINITIVO-->
...
*Escribir aquí la conclusión del pca*
...
```
*Obtenemos que el número óptimo de centroides es 2.*
...
## NbClust 
<!-- VER ESTO QUE DA FALLO AL KNIT TO HTML -->
...
## Conclusión
En conclusión, podemos observar que el número de clústers óptimo sería 2 o 3. Mientras que el método del codo o de la silueta nos indican que debemos dividir en dos grupos, los métodos GAP y NbClust indican que se deben obtener 3 grupos.

Por otro lado, en el clústering aglomerativo cuando dividimos en 3 grupos observamos un notable solapamiento al representar los datos. Sin embargo, con 2 clústers parece que los resultados obtenidos son más claros. Es por ello que concluimos con que el número óptimo de centroides es 3: k=2.
...
Antes de aplicar el proceso de KNN, tenemos que modificar nuestra sección de datos en test, al igual que se ha realizado en train. 
...
fit.knn <- train(gender~., data=df_kvecinos_train, method="knn",
                  metric="F" ,trControl=trainControl,tuneLength = 3)
...
plot(fit.knn)
```
*Para maximizar el F1 Score, utilizando 3 posbibles valores de números de vecinos obtenemos que para 9 vecinos se consigue el mejor valor del F1. RECUERDA: Buscamos maximizar el F1 porque al tener una porporción de 90% hombres y 10% mujeres, queremos predecir con seguridad cuando una edición ha sido realizada por una mujer.*


Realizamos una predicción de k=9 en train.
```{r}
#TRAIN
prediction.knn_train <- predict(fit.knn,newdata=df_kvecinos_train)
cf11 <- confusionMatrix(prediction.knn_train,df_kvecinos_train$gender, mode="everything")
print(cf11)
```
*En la matriz de confusión apreciamos que de 63 mujeres el modelo ha acertado en la predicción para 50 de ellas. Sin embargo, ha realizado 13 predicciones de tipo "hombre" erróneas.*

El valor para el campo f1 es de 0.9479. Comprobamos, calculando "a mano", si la el resultado otorgado es correcto.

```{r}
# Función para calcular f1_score
f1_score <- function(precision, recall) {
  f1 <- (2 * (recall * precision)) / (recall + precision)
  return(f1)
}

print(f1_score(0.9073,0.9922))
```
*Sí parece que se ha calculado correctamente el f1_score para el knn.*

Observemos cómo se comporta el modelo con los datos de test.
```{r}
#TEST
prediction.knn_test <- predict(fit.knn,newdata=df_kvecinos_test)
cf21 <- confusionMatrix(prediction.knn_test,df_kvecinos_test$gender, mode="everything")
print(cf21)
```
*El valor para el F1-Score no disminuye considerablemente con respecto a train. Sin embargo, en la matriz de confusiones se observa una gran diferencia respecto a los resultados obtenidos en train.*

```{r}
print(f1_score(0.8995, 0.9750))
```
*Ratificamos que la medida otorgada por el modelo para F1-Score es precisa y correcta.*
...
knn.k2 <- fit.knn.grid$bestTune 
print(fit.knn.grid)

plot(fit.knn.grid)
```
*Realizamos el mismo método pero con valores de números de vecinos que van desde 1 hasta 30. En este caso obtenemos que el mejor número de vecinos es 10. Aun aumentando considerablemente las posibilidades de elección del modelo de 10 a 30, el modelo escoge un k=10, muy cercano al k=9 escogido anteriormente con dicha restricción.*


Veamos la predicción en train para k=10.
```{r, warning=FALSE}
#TRAIN
prediction.knn2_train <- predict(fit.knn.grid,newdata=df_kvecinos_train,type="prob")[,1]

# Binarizamos a partir de 0.6
clase.pred.knn=ifelse(prediction.knn2_train>0.60,"male","female")

cf12 <- confusionMatrix(as.factor(clase.pred.knn),as.factor(df_kvecinos_train$gender),
      positive="male", mode="everything")

print(cf12)
```
*Se obtiene un F1-Score de 0.9413. Parece que con k=10 se obtienen unos resultados muy similares a los previamente obtenidos con k=9 en train.*

Veamos si en relación a test conseguimos mejores resultados con k=10.
```{r, warning=FALSE}
#TEST
prediction.knn2_test <- predict(fit.knn.grid,newdata=df_kvecinos_test,type="prob")[,1]

# Binarizamos a partir de 0.6
clase.pred.knn2=ifelse(prediction.knn2_test>0.50,"male","female")

cf22 <- confusionMatrix(as.factor(clase.pred.knn2),as.factor(df_kvecinos_test$gender),
      positive="male", mode="everything")

print(cf22)
```
*Con 10 vecinos y ajustando la probabilidad con la que queremos que prediga cada género, obtenemos un F1-Score similar que con k=9. Parece que el modelo conserva el valor de F1-Score de train a test, aunque sí es cierto que la matriz de precisión muestra peores predicciones.*
...
```{r}
#Empleamos train_Data, con todas las variables originales y transformadas. Eliminamos las fechas.
...
```{r}
#TRAIN
prediction1 <- predict(fit_dt1, train_Data, type = 'class')
cf1 <- confusionMatrix(prediction1, train_Data$gender, mode="everything")
print(cf1)
```
*Obtenemos un F1-Score de 0.9509, lo cual es muy positivo. RECUERDA: En nuestro caso, queremos buscar maximizar el F1-Score, es decir, cuando haya algún proyecto/edición en el que participe una mujer tenemos que ser capaces de detectarlo, aunque pequemos de determinar falsos positivos en exceso (proyecto hecho por un hombre que estimamos que ha sido desarrollado por una mujer).*

Comprobemos si se ha calculado correctamente el F1-Score para este modelo.
```{r}
print(f1_score(0.9129,0.9922))
```
...
```{r}
#TEST
prediction1_test <- predict(fit_dt1, test_Data, type = 'class')
cf1_test <- confusionMatrix(prediction1_test, test_Data$gender, mode="everything")
print(cf1_test)
```
...
```{r}
# Eliminamos variables originales, manteniendo sus respectivas transformadas
...
```{r}
prediction2 <- predict(fit_dt2, train_dt, type = 'class')
cf2 <- confusionMatrix(prediction2, train_dt$gender, mode="everything")
print(cf2)
```
*Obtenemos resultados prácticamente idénticos, por lo que las variables que hemos eliminado no afectan notablemente el rendimiento del modelo. Estas son firstDay, lastDay, pagesWomen, wikiprojWomen, weightIJ y NIJ. Nótese que en el caso de pagesWomen y wikiprojWomen, aunque estas variables originales no estén, sí se encuentras sus versiones binarizadas, por lo que, realmente, son importantes para el modelo. *

Veamos qué resultados arroja el modelo con los datos de test.
```{r}
prediction2_test <- predict(fit_dt2, test_dt, type = 'class')
cf2_test <- confusionMatrix(prediction2_test, test_dt$gender, mode="everything")
print(cf2_test)
```
*En test obtenemos un valor de F1 menor que en train, aunque bastante aceptable.*
...
```{r}
# Árbol con hiperparámetros ajustados
...
Veamos cómo qué resultados obtenemos con el árbol con hiperparámetros ajustados.
```{r}
prediction3 <- predict(tune.fit, train_dt, type = 'class')
cf3 <- confusionMatrix(prediction3, train_dt$gender, mode="everything")
print(cf3)
```
*El F1-Score no varía sustancialmente de los resultados obtenidos en predicciones anteriores.*

Veamos en test.
```{r}
prediction3_test <- predict(tune.fit, test_dt, type = 'class')
cf3_test <- confusionMatrix(prediction3_test, test_dt$gender, mode="everything")
print(cf3_test)
```
*El resultado en test para F1-Score es el peor obtenido de todos los árboles anteriores.*


## Bagging (Random Forest) SEGUIR POR AQUI!!!!!

```{r}
set.seed(123) # cada árbol se entrena con una muestra aleatoria (con reemplazo)
# Se pueden usar todo tipo de variables no?
rf <- randomForest(as.factor(train_Data$gender)~., data=train_Data, importance=TRUE,proximity=TRUE) 
print(rf)
```
*conclusiones aqui*
...
Realicemos la predicción en test.
```{r}

test_Data= subset(test_Data, select=-c(pagesWomen, wikiprojWomen))
prediction.rf <- predict(rf, test_Data,type="prob")[,2]
clase.pred.rf=ifelse(prediction.rf>0.5,"2","1")
cf <- confusionMatrix(as.factor(clase.pred.rf), as.factor(test_Data$gender),positive="1")
print(cf)
```
*conclusiones aquí*
...
test_xg <- map_df(test_Data, function(columna) {
  columna %>% 
    as.factor() %>% 
    as.numeric %>% 
    { . - 1 }
...
Una vez hemos realizado el procesamiento de datos pertinente, entrenamos el modelo con nuestra matriz de datos
...
datos$modelo1 <- xgboost(data = datos$train_mat,  
                 objective = "binary:logistic", 
                  ...
                  nthread =2) 
```
*conclusiones aquí* *Observamos que el error incial toma un valor de x. Finalmente, al alcanzar la iteración 10, el error se reduce a x*
...
```
*conclusiones aqui si se ve algo* El número que aparece al lado de cada varable es la calidad, la cual indica la importancia de cada una de estas en cada árbol. A mayor calidad y más arriba, mayor importancia en los árboles usados.

Realizamos un gráfico para visualizar la importancia de cada variable en el modelo
...
Observemos la probabilidad de acabar en cada nodo hoja convirtiendo el valor obtenido en cada uno de ellos. (si hay muchos nodos hoja solamente hacer esto para los que tengan mayor probabilidad)
...
```
Así, en los árboles en los que una observación acabó en la hoja con valor 1.2496, la probabilidad media de que una observación fuera un “yes” en la variable gender es del 22%.
...
datos$predict1 <- predict(datos$modelo1, datos$test_mat)
cbind(datos$predict1 > 0.5, datos$test$gender) %>% 
  data.frame() %>% 
  table() %>% 
...
El clasificador Naive Bayes puede ser de gran utilidad en este dataset, dado el gran número de variables de entrada que presenta y los amplios dominios en los que están definidas. Sin embargo, para variables cuantitativas se asume la distribución normal y sabemos que nuestros datos distan notoriamente de ser normales.

Obtengamos el modelo y realicemos la predicción.
```{r}
# FALLA: VER SI USAR TRAIN_DATA Y TEST_DATA EN BRUTO CON TODO O HACER LIMPIEZA DE ALGUNAS VARIABLES DE LOS DATOS QUE VOY A USAR
model <- naive_bayes(gender ~ ., data = train_Data, usekernel = T) 
# Debemos quitar el target para la predicción:test_Data[,-1] 
probabilities_bayes <- predict(model, test_Data[,-1],type="prob")[,2] 
classes <- as.numeric(probabilities_bayes>0.5)
confusionMatrix(table(classes, test_Data$gender),positive="1") 
```

# Comparación de modelos.
Cada modelo de los empleados funciona de una forma. Algunos necesitan parámetros de entrada o emplean datos aleatorios... Cada uno otorga su propia matriz de confusión, aunque sería útil poder compararlos a todos simultáneamente de alguna forma.

Para ello, representamos la curva ROC.
<!-- NO ejecutar esto hasta que todas las predicciones no funcionen-->

```{r}
#roc_score //no sé si hay que poner esto
par(pty = "s") # square

#XGBoost (no se de donde se saca "probabilities")
roc(test_Data$gender, probabilities,plot=TRUE, legacy.axes = TRUE,
...
# Decision tree
roc(test_Data$gender, prediction2, percent=TRUE, col="goldenrod",lwd= 2,
    print.auc =TRUE, add=TRUE,print.auc.y = 30,plot=TRUE,legend=TRUE)

# Random Forest
roc(test_Data$gender, prediction.rf, percent=TRUE, col="salmon",lwd= 2,
    print.auc =TRUE, add=TRUE,print.auc.y = 20,plot=TRUE,legend=TRUE)

# KNN
roc(test_Data$gender, prediction2.knn, percent=TRUE, col="#977eb8",lwd= 2,
    print.auc =TRUE, add=TRUE,print.auc.y = 10,plot=TRUE,legend=TRUE)


legend("bottom",
       legend=c("XGBOOST", "DT", "RF","KNN", ),
