...
```
*Para maximizar el F1 Score, utilizando 3 posbibles valores de números de vecinos obtenemos que para 9 vecinos se consigue el mejor valor del F1. Buscamos maximizar el F1 porque al tener una porporción de 90% hombres y 10% mujeres, queremos predecir correctamente con la mayor precisión posible cuando una persona va a ser mujer. *
...
prediction <- predict(fit.knn,newdata=df_kvecinos_test)
cf <- confusionMatrix(prediction,df_kvecinos_test$gender)
print(cf)
```
*En la matriz de confusión apreciamos que de 61 mujeres que hay en el conjunto de datos hemos predicho bien a 8, pero que hemos predicho que había 22 mujeres, dejandonos con 14 predicciones de mujer erroneas.*
...
plot(fit.knn.grid)
```
*Realizamos el mismo método pero con valores de números de vecinos que van desde 1 hasta 30, en este caso obtenemos que el mejor número de vecinos es 10.*
...
prediction.knn <- predict(fit.knn.grid,newdata=df_kvecinos_test,type="prob")[,1]

clase.pred.knn=ifelse(prediction.knn>0.60,"male","female")

cf <- confusionMatrix(as.factor(clase.pred.knn), as.factor(df_kvecinos_test$gender),positive="male")

print(cf)
```
*Con 10 vecinos y ajustando la probabilidad con la que queremos que prediga cada género, sacamos uun mejor F1 Score que con el modelo anterior.*
...
```{r}
train_Fit <- rpart(gender~., data = train_Data con variables nuesvascategoricasycontinuas , method = 'class')
rpart.plot(fit.dt, extra = 106)
```

Calculamos nuestra matriz de confusión en train.
```{r}
...
Calculamos nuestra matriz de confusión en test.
```{r}
