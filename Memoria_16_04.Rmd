...
No realizamos un análisis discriminante lineal, puesto que previamente en el EDA se ha observado que no disponemos de datos normales.

## K-Vecinos más cercanos (KNN)


Antes de aplicar al proceso de KNN, tenemos que modificar nuestra sección de datos en test, al igual que se ha realizado en train. 

## Modificación de test_Data
Aplicamos logaritmo a las variables a las que conviene modificar su distribución
```{r}
new_NActDays_test<- log10(test_Data$NActDays+0.5)
new_NPages_test<- log10(test_Data$NPages)
new_ns_talk_test<- log10(test_Data$ns_talk+0.5)
new_ns_userTalk_test<- log10(test_Data$ns_userTalk+0.5)
new_ns_content_test<- log10(test_Data$ns_content+0.5)
new_ns_user_test<- log10(test_Data$ns_user)
new_ns_wikipedia_test<- log10(test_Data$ns_wikipedia+0.5)
```
...
df_test<- cbind(delete_cat,new_NActDays_test, new_NPages_test,new_ns_talk_test, new_ns_userTalk_test, 
                  new_ns_content_test,new_ns_user_test,new_ns_wikipedia_test)
...
<!-- HAY QUE PONER F1-SCORE EN VEZ DE ACCURACY -->
...
Realizamos una predicción de k en test.
```{r}
#USAR DATOS DE TEST
...
Realizamos una predicción en test para el k óptimo.
```{r}
#USAR DATOS DE TEST
...
## Árboles de decisión
Aplicamos el método de DT al conjunto de datos en train. Incluimos tanto variables categóricas como continuas.
...
cf1 <- confusionMatrix(prediction1, train_Data$gender)
print(cf1)
```
*Obtenemos un accuracy del 90%, lo cual es positivo. En nuestro caso, queremos buscar maximizar el F1-Score, es decir, cuando haya algún proyecto/edición en el que participe una mujer tenemos que ser capaces de detectarlo, aunque pequemos de determinar falsos positivos en exceso (proyecto hecho por un hombre que estimamos que ha sido desarrollado por una mujer).*

Atendiendo al árbol anterior, parece interesante binarizar la variable weightIJ, dado que tiene muchas categorías y, como está en el árbol, facilitaría su comprensión si solo tuviese 2 categorías. Sin embargo, weightIJ y NIJ son una forma de categorizar la variable NEds. 

Como NEds ya está incluida, eliminamos weightIJ y NIJ para representar el árbol. Además, sustituimos las variables originales por sus transformadas, en vez de incluirlas todas juntas.
```{r}
...
Realizamos la predicción con este ajuste.
```{r}
prediction2 <- predict(fit_dt2, train_dt, type = 'class')
cf2 <- confusionMatrix(prediction2, train_Data$gender)
print(cf2)
```
*Obtenemos resultados prácticamente idéntidos, por lo que las variables que hemos eliminado no afectan notablemente el rendimiento del modelo. Estas son firstDay, lastDay, pagesWomen, wikiprojWomen, weightIJ y NIJ. Nótese que en el caso de pagesWomen y wikiprojWomen, aunque estas variables originales no estén, sí se encuentras sus versiones binarizadas, por lo que, realmente, son importantes para el modelo.*

Calculemos nuestra matriz de confusión en test con el primer ajuste realizado (el que usa todo train_Data).
```{r}
prediction3 <- predict(fit_dt1, test_Data, type = 'class')
cf3 <- confusionMatrix(prediction3, train_Data$gender)
print(cf3)
```
*En test obtenemos x rendimiento que en train. seguir por aqui.*
Ajustando los hiperparámemtros del modelo podemos obtener resultados específicos que puedan resultar en una mejor explicabilidad del mismo. En nuestro caso representemos el árbol con profundidad máxima y con la condición de que debe existir un número mínimo de 50 observaciones en un nodo para que se intente una división.

```{r}
# Árbol con hiperparámetros ajustados
control <- rpart.control(minsplit=50,cp = 0)
tune.fit <- rpart(gender~., data = train_Data, method = 'class', control = control)
rpart.plot(tune.fit, extra = 106)
```
*Obtenemos un árbol mucho más profundo. Al igual que los anteriores tiene a wikiprojWomen y Ndays como variables principales. Más adelante veremos qué rendimiento obtenemos con él.*

Veamos cómo qué resultados obtenemos con el árbol con hiperparámetros ajustados.
```{r}
prediction4 <- predict(tune.fit, train_Data, type = 'class')
cf4 <- confusionMatrix(prediction4, train_Data$gender)
print(cf4)
```
*El accuracy sigue siendo del 90%, aunque hay un mayor porcentaje de falsos positivos (proyectos hechos por hombre falsamente declarados como hechos por mujer), un 46,75%.*
...
# Debemos quitar el target para la predicción. Por eso test_D
probabilities <- predict(model, test_Data[,-1],type="prob")[,2] 
classes <- as.numeric(probabilities>0.5)
confusionMatrix(table(classes, test_Data$gender),positive="1") # falla esta línea
