...
```
*Hay 16 grupos al igual que en weightIJ*
*Las variables E_Bpag y E_NEds están relacionadas dos a dos, las variables NIJ y weightIJ también están relacionadas dos a dos.*
```{r}
...
```{r}
#Histograma para ver como están repartidos los grupso en weightIJ.
...
*Binarizaremos la varibale porque la gran mayoría de los datos toman valor 0.*
```{r}
...
```
```{r}
fviz_cluster(list(data=train_Kmeans_std,cluster=sub_grp))
```
