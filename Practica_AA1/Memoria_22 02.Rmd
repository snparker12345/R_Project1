...
* weightIJ: Corrección de peso para estrato ij. <!--NO ENTIENDO AUN LAS UNIDADES DE ESTO NI LO QUE SIGNIFICA EXACTAMENTE-->
...
## firstDay
Hemos dividido esta variable en 3 datos, siendo estos el primer año de edicion, el primer mes y el primer día.
```{r}
#Separamos las fechas de los datos
ano_inicial<-as.numeric(substr(train_Data$firstDay,start = 1, stop = 4 ))
#Resumen de la variable
summary(ano_inicial)
```
*La media coincide con la mediana por lo que se trata de una variable simétrica, y el primer y tercer cuartil presentan valores típicos.*
...
*Se pueden apreciar dos poblaciones, una concentrada en 2007 y otra en 2012.*
```{r}
#Año inicial - PDF
pdf(ano_inicial, x_label="Año")
```
</center>\
*No se consiguen distinguir dos poblaciones.*
...
summary(mes_inicial)
```
*La mediana está muey cerca de la media por lo que se trata de otra variable muy simétrica. Los cuartiles toman valores típicos.*
...
*El mes de Agosto es en el que mas nuevas cuentas se han creado, no hay nada más destacable*
```{r}
#Boxplot - Mes inicial
...
*La media y la mediana son parecidas, lo que indica simetría*
...
summary(ano_final)
```
*La media vuelve a coincidir con la mediana, aunque esta vez el tercer cuartil es elevado (2017, el dato más alto), lo que nos puede indicar una asimetría hacia el final.*
...
*Observamos que los primeros años son bastante uniformes, solo se aprecia un aumento leve, pero en el 2017 se duplican la cantidad de ultimas ediciones con respecto al año anterior.Y a diferencia del año inicial, aqui no hay sospechas de haber dos poblaciones*
...
```
*Se vuelven a observar valores estandar.*
```{r}
histogram(dia_final, x_label = "Ultimo dia")
```
</center>\
*Se observa una concentración de últimas ediciones en el primer día del mes. Y que el día con menos cantidad es el 31, lo cual era de esperar, vuelve a ser sorprendete que la diferencia entre el día 31 y el resto de días sea tan pequeña.*
```{r}
#Dia final - Boxplot
...
```
Observamos que el primer cuartil es bajo, por lo que tenemos una gran concentración de datos con valores bajos. La media es muy superior a la mediana, lo que va a presentar asimetría a la derecha. También se observa que el máximo de ediciones es (153193), dato que es erroneo.
...
```
*Se puede ver que la probabiidad es mayor cerca de la mediana, y a desde el máximo de probabilidad esta dismiuye hasta sus mínimos (excepto en el dato aítpico).*
...
```
*La media y la mediana son prácitamente idénticas, por lo que presentará simetría.El tercer cuartil presenta un valor bajo, por lo que no habrá gran concetración de valores altos.*
```{r}
#weightIJ - Histograma
...
```
*Se observan datos atípicos tanto en valores bajor y altos.*
```{r}
#weightIJ - PDF
