...
*Prácticamente todos los valores se encuentran en 0 debido a que el tercer cuartil es, la media no es 0 porque hay un valor máximo muy elevado.*
```{r}
...
*Se observa algunos datos superiores a Q3. Solo se encuentra un único dato atípico. Aunque el valor máximo difiera mucho del segundo valor más alto (valor máximo = 762, segundo valor max = 20) no podemos aumir que se trate de un dato erróneo.*
...
*Observamos bastantes datos superiores a Q3 + 1.5RI. Sin embargo, solo se considerarán atípicos datos con un calor superior a 6000.*
...
*Obtenemos un valor de lambda = 0. -> TRANSFORMACIÓN: log10(x) (Le sumaremos 0.5 a los datos porque toma valores comenzando en 0).*
```{r}
...
```
*El primer y tercer cuartil toman valores muy bajos mientras que el máximo es muy alto en comparación, y al ser la media muy superior a la medaiana se espera uan asimetría a la derecha.*
```{r}
