...
*Los días con ediciones por hombres y páginas diferentes editadas por hombres son algo mayores que sus respectivos femeninos. Observemos si la diferencia es significativa.*
...
En este caso, al tratarse de variables binarias, representamos los diagramas de barras y contrastamos con el test de chi-cuadrado.
...
```
*Observamos que en el primer caso no se rechaza la hipótesis nula (alpha=0.05 por defecto), por lo que hay relación significativa entre pages_Women_bin y el target. Sin embargo, en el caso de wikiprojWomen_bin, no se observa relación alguna.*

```{r}
plot(train_Data$NDays,train_Data$gender, xlab = "Hab_medio_med", ylab= "Gender",main="Scatter Plot")
